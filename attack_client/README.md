...
python attack_client/attack_weakcurve.py     # Pohlig-Hellman → rút sạch Carlos
...
|--------|-----------------|----------|-----|--------|
| `attack_nonce_reuse.py` | `/api/transactions`, `/api/accounts`, `/api/state` | Giải hệ phương trình từ 2 chữ ký cùng `r` của cùng người gửi | `/api/tx/submit` (chữ ký tự tạo) | Khôi phục khóa ví lặp nonce (`alice`) → rút về `bob` |
| `attack_psychic.py` | — | Token JWT với chữ ký `(0,0)` | `/api/admin/users` + cookie | Chiếm admin, lộ CCCD/SĐT/email các tài khoản |
| `attack_weakcurve.py` | `/api/accounts` | **Pohlig-Hellman** giải ECDLP (bậc 256-bit nhưng TRƠN) | `/api/tx/submit` | Khôi phục khóa Carlos → rút 5.000.000 |

> `attack_weakcurve.py` mất **~10-15 giây** (Pohlig-Hellman + BSGS thật). Đường cong của
> Carlos trông y như thật (dạng `y²=x³+b` như secp256k1, `p` và `n` đều ~256 bit),
